...
public class AdminCalendarController {
    ...
    public void initialize() {
        ...
        if (currentAdmin != null) {
            ...
            adminClassrooms = ClassroomService.getAdminClassrooms(currentAdmin.getId());
        }
    ...
    public void goToDashboard() {
        navigateToView("/fxml/admin-dashboard.fxml");
    }
    ...
    private VBox createDayCell(LocalDate date) {
        ...
        for (Routine routine : dayRoutines) {
            if (displayCount >= maxDisplay) break;
            HBox eventBox = createEventIndicator(routine.getCourseName(), "#9333ea");
            cell.getChildren().add(eventBox);
            displayCount++;
        }
    ...
    private List<CalendarEvent> getEventsForDate(LocalDate date) {
        ...
        if (adminClassrooms != null) {
            for (Classroom classroom : adminClassrooms) {
